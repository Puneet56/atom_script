...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{
			`{"name": "Monkey"}[reaction(x) { x }];`,
			"unusable as hash key: REACTION",
		},
	}
...
func TestArrayIndexExpressions(t *testing.T) {
	tests := []struct {
		...
		{
			"atom myArray = [1, 2, 3]; myArray[2];",
			3,
		},
		{
			"atom myArray = [1, 2, 3]; myArray[0] + myArray[1] + myArray[2];",
			6,
		},
		{
			"atom myArray = [1, 2, 3]; atom i = myArray[0]; myArray[i]",
			2,
		},
		{
			"[1, 2, 3][3]",
